...
class ResultSaver:
    def __init__(self, result_dir='C:/Users/Linfe/OneDrive/Desktop/Seg/result'):
...
class Trainer:
    def __init__(self, model, train_dataloader, val_dataloader, optimizer, loss_func, device,
                 result_dir='C:/Users/Linfe/OneDrive/Desktop/Seg/result', checkpoint_path='./unet.pkl'):
        self.model = model
    ...
    def run(self, epochs):
        ...
        for epoch in range(epochs):
            ...
            image = image[0]  # Extract a single image from the batch
            mask_rgb = self.train_dataloader.dataset.class_to_rgb(mask.squeeze(0)).to(self.device)
            pred_rgb = pred_rgb[0]
            comparison_path = result_saver.save_comparison(image.cpu(), mask_rgb.cpu(), pred_rgb.cpu(), epoch, idx)
        ...
        '''
        epoch = 3
        train_loss.append(0.13)
        train_loss.append(0.11)
        train_loss.append(0.9)
        train_loss.append(0.3)
        train_loss.append(0.3)
        val_loss = [2, 3, 4, 5, 6]
        '''
        plot_loss(train_loss, val_loss)
